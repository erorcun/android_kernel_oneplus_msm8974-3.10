...
#include "../sensor/io/msm_camera_i2c_mux.h"
	...
	struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	rc = msm_sensor2_get_sub_module_index(of_node, &sensordata->sensor_info);
	if (rc < 0) {
		pr_err("%s failed %d\n", __func__, __LINE__);
		goto FREE_SENSORDATA;
	}
	...
	rc = msm_sensor2_get_dt_csi_data(of_node, &sensordata->csi_lane_params);
	if (rc < 0) {
		pr_err("%s failed %d\n", __func__, __LINE__);
		goto FREE_SENSOR_INFO;
	}

	rc = msm_camera2_get_dt_vreg_data(of_node,
	...
	if (gpio_array_size) {
		...
		rc = msm_camera2_get_dt_gpio_set_tbl(of_node, gconf,
			gpio_array, gpio_array_size);
		if (rc < 0) {
			pr_err("%s failed %d\n", __func__, __LINE__);
	...
	rc = msm_sensor2_get_dt_actuator_data(of_node,
...
int32_t msm_sensor2_free_sensor_data(struct msm_sensor_ctrl_t *s_ctrl)
{
...
int msm_sensor2_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	struct msm_camera_power_ctrl_t *power_info;
	...
	return msm_camera2_power_down(power_info, sensor_device_type,
		sensor_i2c_client);
}

int msm_sensor2_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	int rc;
	struct msm_camera_power_ctrl_t *power_info;
	struct msm_camera_i2c_client *sensor_i2c_client;
	...
	rc = msm_camera2_power_up(power_info, s_ctrl->sensor_device_type,
		sensor_i2c_client);
	if (rc < 0)
		return rc;
	rc = msm_sensor2_check_id(s_ctrl);
	if (rc < 0)
		msm_camera2_power_down(power_info, s_ctrl->sensor_device_type,
					sensor_i2c_client);

	return rc;
}

int msm_sensor2_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	int rc = 0;
	uint16_t chipid = 0;
	struct msm_camera_i2c_client *sensor_i2c_client;
...
int msm_sensor2_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
...
int msm_sensor2_check_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	int rc;
	...
	if (s_ctrl->func_tbl->sensor_match_id)
		...
		rc = msm_sensor2_match_id(s_ctrl);
	if (rc < 0)
		pr_err("%s:%d match id failed rc %d\n", __func__, __LINE__, rc);
	return rc;
}

static int msm_sensor2_power(struct v4l2_subdev *sd, int on)
{
	int rc = 0;
	struct msm_sensor_ctrl_t *s_ctrl = get_sctrl(sd);
	mutex_lock(s_ctrl->msm_sensor_mutex);
...
static struct msm_sensor_fn_t msm_sensor_func_tbl = {
	.sensor_config = msm_sensor2_config,
	.sensor_power_up = msm_sensor2_power_up,
	.sensor_power_down = msm_sensor2_power_down,
	.sensor_match_id = msm_sensor2_match_id,
};
...
int32_t msm_sensor2_platform_probe(struct platform_device *pdev, void *data)
{
	int rc = 0;
	struct msm_sensor_ctrl_t *s_ctrl =
...
int msm_sensor2_i2c_probe(struct i2c_client *client,
