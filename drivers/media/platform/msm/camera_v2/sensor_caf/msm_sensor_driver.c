...
static int msm_sensor_platform_remove(struct platform_device *pdev)
{
	...
	msm_sensor2_free_sensor_data(s_ctrl);
...
const struct of_device_id msm_sensor_driver_dt_match[] = {
...
int32_t msm_sensor_driver_probe(void *setting)
{
	...
	rc = msm_camera2_fill_vreg_params(
		power_info->cam_vreg,
		power_info->num_vreg,
	...
	struct device_node *of_node)
{
	...
	rc = msm_camera2_get_dt_gpio_req_tbl(of_node, gconf, gpio_array,
		gpio_array_size);
	if (rc < 0) {
		pr_err("failed");
		goto FREE_GPIO_CONF;
	...
	rc = msm_camera2_init_gpio_pin_tbl(of_node, gconf, gpio_array,
		gpio_array_size);
	if (rc < 0) {
		pr_err("failed");
